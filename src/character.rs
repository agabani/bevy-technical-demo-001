...
fn spawn(
    ...
    characters: Query<(&Character, &network::ConnectionId)>,
    connections: Query<(&network::Connection, &network::ConnectionId)>,
) {
...
fn spawned(mut commands: Commands, mut events: EventReader<protocol::Endpoint<protocol::Spawned>>) {
    for event in events.iter() {
        ...
        let entity = commands
            ...
            .insert(network::ConnectionId(event.connection_id))
            .id();
